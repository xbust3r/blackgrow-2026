...
- [ ] **Extender el mixin `field` con `textarea`.** Hoy
...
- [ ] **`icon-with-text.pug`** — icono, título y texto en bloque.
...
- [ ] **`text-marquee.pug`** — banda de texto que se desplaza en horizontal, en
...
- [ ] **`interactive-link-showcase.pug`** — lista de enlaces que cambia la
...
- [ ] **`contact-form.pug`** — nombre, email, teléfono y comentario.
...
- [ ] **`google-map.pug`** — mapa incrustado de Contacto.
...
- [ ] **`blog-card.pug`** — tarjeta de artículo: imagen, categoría, fecha,
...
- [ ] **Formatos de entrada.** El formato cambia **qué encabeza la tarjeta**,
      no el resto. Resolverlo con un parámetro `formato` dentro de
      `blog-card.pug`, no con seis archivos:

      - [ ] `standard` — imagen destacada.
      - [ ] `quote` — cita con su autoría, sin imagen.
      - [ ] `link` — enlace destacado, sin imagen.
      - [ ] `gallery` — varias imágenes en rejilla.
      - [ ] `audio` — `<audio controls>` nativo, sin librería.
      - [ ] `video` — contenedor con proporción para el vídeo incrustado.
...
- [ ] **`blog-sidebar.pug`** — tres widgets: categorías, últimas entradas y
      redes sociales. Un mixin por widget dentro del mismo archivo.

- [ ] **`pagination.pug`** — paginación numerada, con página actual marcada
      (`aria-current='page'`) y enlaces anterior/siguiente. Sin «cargar más» ni
      scroll infinito: el origen no los tiene.

- [ ] **`comment-form.pug`** — comentario, nombre, email, web y casilla de
      consentimiento. Con `js-form` y `+field`, valida solo. `action` vacío.

- [ ] **`comment-list.pug`** — lista de comentarios con avatar, autor, fecha,
...
- [ ] **`blog.pug`** — listado con barra lateral **a la derecha**.
- [ ] **Variante barra a la izquierda.** No es una página nueva: es la misma
      plantilla con el lado como parámetro. Resolverlo con una variable, no
      duplicando el archivo.
- [ ] **Variante sin barra**, a ancho completo. Mismo parámetro.
- [ ] **`blog-masonry.pug`** — mampostería a 3 / 3 / 2 / 1 columnas.
      **Se resuelve con `columns` de CSS**, no con isotope ni JavaScript: el
      origen usa una librería que aquí no hace falta.
- [ ] **`blog-single.pug`** — artículo: cabecera con título, autor y fecha;
...
- [ ] **`contact.pug`** — usa `contact-form` y `google-map`.
      Referencia: <https://playgrow.qodeinteractive.com/contact-us/>.

- [ ] **`faq.pug`** — preguntas frecuentes.
...
- [ ] **`kids-store.pug`** — testimonios, icono-con-texto y productos.
- [ ] **`baby-shop.pug`** — marquesina, showcase interactivo y galería.
- [ ] **`shop-grid.pug`** — rejilla de producto como protagonista.
- [ ] **`landing.pug`** — imagen-con-texto, marquesina, galería e iconos.
...
- [ ] **Todo componente nuevo aparece en `src/pages/components.pug`**, con su
      nombre visible. Es donde se comprueban los que no tienen página propia.
- [ ] **Toda página nueva extiende `src/layouts/main-template.pug`** y define su
      propio `block title` y descripción. Dos páginas con el mismo `<title>` es
      un fallo.
- [ ] **Validaciones en verde**, en este orden:
      ```bash
      pnpm lint
      pnpm validate
      pnpm build
      pnpm preview
      ```
- [ ] **Comprobado servido y con teclado**, en 360 y en 1200. Redimensionando el
      viewport de verdad: estrechar un elemento no reevalúa las media queries.
- [ ] **Informe** en la raíz: qué se hizo, qué se supuso, qué quedó pendiente y
